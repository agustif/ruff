...
#[test]
fn check_json_output_no_errors() -> anyhow::Result<()> {
    let case = TestCase::with_files([("no_error.py", "a: int = 1")])?;
    let mut cmd = case.command(); // `case.command()` already adds "check"
    cmd.arg("--output-format")
        .arg("json")
        .arg("no_error.py"); // Path relative to project_dir

    let output = cmd.output().context("Failed to execute command")?;

    assert!(
        output.status.success(),
        "Command should succeed. Stderr: {}, Stdout: {}",
        String::from_utf8_lossy(&output.stderr),
        String::from_utf8_lossy(&output.stdout)
    );

    let stdout = String::from_utf8(output.stdout)
        .context("Failed to read stdout as UTF-8")?
        .trim()
        .to_string();
    assert_eq!(stdout, "[]", "Expected empty JSON array for no errors");
